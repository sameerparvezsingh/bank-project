...
const loginTable='logindetails';
const currencyExchangeTable='currencyexchange';
//query functions
...
const getCurrencyExchangeRate = (toCurrency) => new Promise((resolve,reject)=>{
    let queryStatementToGetAccounts = `SELECT value from ${currencyExchangeTable} WHERE tocurrency=$1`
...
const getAvailabilityStatusOfUserInDatabase = (useridToValidate) => new Promise((resolve,reject)=>{
  let queryStatementForValidatingAccountNumber = `select exists(select 1 from ${loginTable} where userid= $1) AS "exists"`
  databaseConnection.pool.query(
  queryStatementForValidatingAccountNumber,
  [useridToValidate],
  (error, results) => {
...
const getLoginDetails = (userIdToAuthenticate) => new Promise((resolve,reject)=>{
  let queryStatementToGetAccounts = `SELECT password from ${loginTable} WHERE userid=$1`
